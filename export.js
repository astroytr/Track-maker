...
function buildExportCode() {
  ...
  return `// ═══════════════════════════════════════════════════
// ${name}
// Generated by Circuit Forge v7.1
// Upload as: track_${key}.js  (drop into your racing game folder)
// Registers as: '${key}'  (matches how the engine discovers track files)
// ═══════════════════════════════════════════════════
(function () {
  'use strict';
...
${wpLines}
    ...
    // ── Centreline — used by the engine to build the road mesh
    // and place the car at the start/finish line.
    // This is the same waypoint data scaled to game-world units.
    build: function () {
      return [
${wpLines}
...
${barrierCode}
  ...
  if (typeof registerTrack === 'function') {
    // Derive the key from this file's actual filename at runtime so it works
    // regardless of what the file was renamed to when uploaded.
    // e.g.  track_monaco.js  →  'monaco'
    //        track_my_test.js →  'my_test'
    var _src = (typeof document !== 'undefined')
      ? (document.currentScript && document.currentScript.src || '')
      : '';
    var _match = _src.match(/track_([^\/]+)\.js/);
    var _key = _match ? _match[1] : '${key}';
    registerTrack(_key, trackData);
